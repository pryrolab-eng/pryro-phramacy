...
import { InvoicesTable, BillingStatCard } from '@/components/subscription'
...
export default function PharmacyBillingPage() {
  ...
  const handleSubscribe = async (plan: SubscriptionPlan) => {
    ...
    try {
      ...
      toast.success(`Subscribed to ${plan.name}`, {
        id: tid,
        description: `Billing cycle: ${billingCycle}`,
      })
    } catch (err) {
  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      const res = await fetch('/api/saas/invoice', {
        method: 'POST',
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage your plan, branches, and invoices</p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <BillingStatCard
          ...
          sub={summary?.can_add_branch ? 'Can add more' : 'At limit'}
          accent="green"
        />
        <BillingStatCard
      ...
      <Tabs defaultValue="plan">
        <TabsList>
          ...
          <TabsTrigger value="upgrade" data-value="upgrade">Upgrade / Change</TabsTrigger>
            ...
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center justify-center py-12 gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground text-sm mt-1">Choose a plan below to unlock all features</p>
                </div>
              </CardContent>
            </Card>
          )}
                  ...
                  {summary.branch_subscriptions.map(sub => (
                    <div key={sub.id} className="flex items-center justify-between p-3 border rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" size="sm" className="text-red-500 h-7" onClick={() => setCancelTarget(sub.id)}>
                          Cancel
                        </Button>
                      </div>
                    </div>
        ...
        <TabsContent value="upgrade" className="mt-6">
          ...
          {!plansQuery.isError && plans.length === 0 && !plansQuery.isFetching && (
            <p className="text-sm text-muted-foreground text-center py-8">No subscription plans available yet.</p>
          )}
            ...
            <AlertDialogDescription>
              {upgradeTarget?.price === 0
                ...
                : billingCycle === 'yearly' && (upgradeTarget?.yearly_price ?? 0) > 0
                  ? `You will be charged RWF ${(upgradeTarget!.yearly_price!).toLocaleString()} per year. Your current plan will be cancelled immediately.`
                  : `You will be charged RWF ${Number(upgradeTarget?.price ?? 0).toLocaleString()} per month. Your current plan will be cancelled immediately.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => upgradeTarget && void handleSubscribe(upgradeTarget)} disabled={subscribe.isPending}>
...
function PlanCard({
  ...
  const yearlySavings = plan.price > 0 && yearlyPrice > 0 ? Math.round(plan.price * 12) - yearlyPrice : 0
    ...
    <Card className={`relative ${plan.is_popular ? 'border-2 border-blue-600' : ''} ${isCurrent ? 'ring-2 ring-green-500' : ''}`}>
      ...
      <CardHeader>
        ...
        <div className="text-3xl font-bold">
          ...
          {displayPrice > 0 && <span className="text-sm font-normal text-muted-foreground">/{displayPeriod}</span>}
        </div>
        {showYearly && yearlySavings > 0 && (
          <p className="text-xs text-green-600 font-medium">Save RWF {yearlySavings.toLocaleString()} vs monthly</p>
        )}
      </CardHeader>
      <CardContent className="space-y-4">
